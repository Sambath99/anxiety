...
function addToCart() {
    ...
    const item = {
        ...
        quantity: quantity,
        price: price,
        image: './detail-image/hoodie/hoodie-012/012front.JPEG', // Update with correct image path
        type: 'hoodie'
    };
    ...
    showToast(`Added ${quantity} Hoodie${quantity > 1 ? 's' : ''} (${selectedSize}) to cart`);
...
async function fetchStockStatus() {
    try {
        ...
        // Get the current hoodie ID from the file name
        const currentPage = window.location.pathname;
        let hoodieId;
        
        if (currentPage.includes('006')) {
            hoodieId = 'hoodie006';
        } else if (currentPage.includes('007')) {
            hoodieId = 'hoodie007';
        } else if (currentPage.includes('012')) {
            hoodieId = 'hoodie012';
        } else if (currentPage.includes('013')) {
            hoodieId = 'hoodie013';
        }
        
        if (data.products && data.products[hoodieId]) {
            const sizeButtons = document.querySelectorAll('.size-btn');
            sizeButtons.forEach(button => {
                const size = button.dataset.size;
                if (!data.products[hoodieId].sizes[size]) {
                    button.classList.add('disabled');
                }
            });
            
            return data.products[hoodieId].stock_status;
        } else {
