...
function addToCart() {
    ...
    const item = {
        ...
        quantity: quantity,
        price: price,
        image: './detail-image/shirt/shirt-017/017.png', // Update with correct image path
        type: 'shirt'
    };
...
async function fetchStockStatus() {
    try {
        ...
        if (data.products && data.products.shirt017 && data.products.shirt017.sizes) {
            const sizeButtons = document.querySelectorAll('.size-btn');
            sizeButtons.forEach(button => {
                const size = button.dataset.size;
                if (!data.products.shirt017.sizes[size]) {
                    button.classList.add('disabled');
                }
            });
            
            return data.products.shirt017.stock_status;
        } else {
