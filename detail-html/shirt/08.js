...
function addToCart() {
    ...
    const item = {
        ...
        quantity: quantity,
        price: price,
        image: './detail-image/shirt/shirt-08/008front.png', // Update with correct image path
        type: 'shirt'
    };
...
async function fetchStockStatus() {
    try {
        ...
        if (data.products && data.products.shirt08 && data.products.shirt08.sizes) {
            const sizeButtons = document.querySelectorAll('.size-btn');
            sizeButtons.forEach(button => {
                const size = button.dataset.size;
                if (!data.products.shirt08.sizes[size]) {
                    button.classList.add('disabled');
                }
            });
            
            return data.products.shirt08.stock_status;
        } else {
