...
function addToCart() {
    ...
    const item = {
        ...
        quantity: quantity,
        price: price,
        image: './detail-image/shirt/shirt-011/011front.JPEG', // Update with correct image path
        type: 'shirt'
    };
...
async function fetchStockStatus() {
    try {
        ...
        if (data.products && data.products.shirt011 && data.products.shirt011.sizes) {
            const sizeButtons = document.querySelectorAll('.size-btn');
            sizeButtons.forEach(button => {
                const size = button.dataset.size;
                if (!data.products.shirt011.sizes[size]) {
                    button.classList.add('disabled');
                }
            });
            
            return data.products.shirt011.stock_status;
        } else {
