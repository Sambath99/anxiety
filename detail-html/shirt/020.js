...
function addToCart() {
    ...
    const item = {
        ...
        quantity: quantity,
        price: price,
        image: './detail-image/shirt/shirt-020/A20.png', // Update with correct image path
        type: 'shirt'
    };
...
async function fetchStockStatus() {
    try {
        ...
        if (data.products && data.products.shirt020 && data.products.shirt020.sizes) {
            const sizeButtons = document.querySelectorAll('.size-btn');
            sizeButtons.forEach(button => {
                const size = button.dataset.size;
                if (!data.products.shirt020.sizes[size]) {
                    button.classList.add('disabled');
                }
            });
            
            return data.products.shirt020.stock_status;
        } else {
