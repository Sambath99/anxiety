...
function addToCart() {
    ...
    const item = {
        ...
        quantity: quantity,
        price: price,
        image: './detail-image/hoodie/hoodie-006/006front.png', // Update with correct image path
        type: 'hoodie'
    };
...
async function fetchStockStatus() {
    try {
        ...
        if (data.products && data.products.hoodie006 && data.products.hoodie006.sizes) {
            const sizeButtons = document.querySelectorAll('.size-btn');
            sizeButtons.forEach(button => {
                const size = button.dataset.size;
                if (!data.products.hoodie006.sizes[size]) {
                    button.classList.add('disabled');
                }
            });
            
            return data.products.hoodie006.stock_status;
        } else {
